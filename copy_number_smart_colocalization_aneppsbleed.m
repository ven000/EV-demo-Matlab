...
pathnam='C:\Users\80933\Desktop\newflowdata2sr'; % input for data file path, only one step needs user input.
...
threshold1 = baseline1+8*mad(data1,1);
...
if B(t,1)>B(t-3,1) & B(t,1)>B(t-2,1) & B(t,1)>=B(t-1,1) & B(t,1)>B(t+1,1) & B(t,1)>B(t+2,1) & B(t,1)>B(t+3,1) & B(t,1)>threshold1 & B(t,1)<threshold1_upper;
B(t,3)=B(t,1);
B(t-3,3)=0;
B(t-2,3)=0;
B(t-1,3)=0;
...
index1 = find(B(:,3)>0); %index1 is for peak position in trajectory 1.

%data2=data2(1:1200000);
figure(1)
plot(data1)
hold on
plot(data2)
baseline2 = mode(data2);
threshold2 = baseline2+8*mad(data2,1); %Run flowhist first to determine threshold for peak finding.
B(:,1)=data2;
...
index2 = find(B(:,3)>0);%index2 is for peak position in trajectory 2.
...
colocalize1_t = zeros(1,length(index1));%Empty array for colcolized event index in data1(membranedye).
colocalize2_t = zeros(1,length(index1));%Empty array for colcolized event index in data2(antibody).
...
%          if index2(j)+delay+right>=index1(i)&&index1(i)>=index2(j)+delay-left % shift the two trajectories for colocalization.Comments and uncomments to adjust delay time range based on correlation plot.
%              colocalize1_t(i)=index1(i);
%              colocalize2_t(i)=index2(j);  
%              
%          end
%          
%     end
...
for j=1:length(index2)
    for i=1:length(index1)
         if index2(j)+delay+right>=index1(i)&&index1(i)>=index2(j)+delay-left % shift the two trajectories for colocalization.Comments and uncomments to adjust delay time range based on correlation plot.
             colocalize1_t(j)=index1(i);
             colocalize2_t(j)=index2(j);
...
colocalize1 = zeros(1,length(index1));%Empty array for colcolized event index in data1(membranedye).
colocalize2 = zeros(1,length(index1));%Empty array for colcolized event index in data2(antibody).


%%%%%%%index1 remove too close peaks in channel E for beads, eg. shoulder peak
disp(length(index1));
...
for i=1:(length(index1)-1)
    if index1(i+1)-index1(i)<=window %based on bin width of peaks
        if data1(index1(i+1))>=data1(index1(i))
            index1(i)=0;
        else
            index1(i+1)=0;
        end
...
for j=1:(length(index2)-1)
    if index2(j+1)-index2(j)<=window %based on bin width of peaks
        if data2(index2(j+1))>=data2(index2(j))
            index2(j)=0;
        else
            index2(j+1)=0;
        end
...
disp(length(index1)/length(data1)*10000);%hz
disp(length(index2)/length(data1)*10000);%hz
...
for j=1:length(index2)
    for i=a:length(index1)
         if index2(j)+delay_p+right>=index1(i)&&index1(i)>=index2(j)+delay_p-left % shift the two trajectories for colocalization.Comments and uncomments to adjust delay time range based on correlation plot.
             colocalize1(j)=index1(i);
             colocalize2(j)=index2(j);
             a=i+1;
             break
...
sumi2(1,i)=(data2(realevent(i))-baseline2)+(data2(realevent(i)-1)-baseline2)+(data2(realevent(i)+1)-baseline2);
end
...
ratio=zeros(1,length(realevent));
for i=1:length(realevent1)
%sumi1(1,i)=data1(realevent1(i))-baseline1;
sumi1(1,i)=(data1(realevent1(i))-baseline1)+(data1(realevent1(i)-1)-baseline1)+(data1(realevent1(i)+1)-baseline1);
delay_t(1,i)=realevent1(i)-realevent(i);
%ratio(1,i) = sumi2(1,i)/sumi1(1,i);
end

% %for 5/8% bleed through correction, intensity ratio is 0.08
% r = randperm(length(realevent),int16(length(index1)*0.34));
% for i = 1:length(r);
%    sumi2(1,r(i))= sumi2(1,r(i))-sumi1(1,r(i))* 0.06; 
% end



figure(90)
scatter(index1,(data1(index1)+data1(index1-1)+data1(index1+1)-3*baseline1));
hold on
scatter(realevent1,sumi1,'+');
legend
title('intensity of anepps')
xlabel('time (bin)')
ylabel('intensity')
...
xlabel('ratio')
ylabel('count')
disp([mean(ratio),median(ratio),mode(ratio)]);
...
% bleed = find (sumi1*0.14+10.22-sumi2>0);
% disp(length(bleed)/length(index1));
...
colocalpercentage = length(realevent)/length(index1);%index1 is for peak position in trajectory 1. membrane dye
expresspercentage = (length(realevent)-length(index1)*length(index2)*delaywidth/length(data2))/length(index1)/(1-length(index1)*delaywidth/length(data2));%
randompercentage = (length(index2)-length(index1)*expresspercentage)*delaywidth/length(data2);
...
interval = 1;
bins = 0:interval:max(sumi2); %Default bin number is 50, modify it to change bin size.
...
% sumi3=zeros(1,length(index2));

% index_ab = setxor(index2,realevent);
% 
...
%%%%%%%%%%%%%%%%%%%%%%%
%use single PE file to do fitting and deconvolution
...
data9=fscanf(fileID,'%d');
%data=data(1:600000);
%data=data(600001:1200000);
%data=data(1200001:1800000)'; %Uncomment if you want to crop the trajectory.

foldBest = 8; % the fold of mad for setting threshold in part 1

singleab = thresh_fit2(foldBest, foldBest, 10, data9)
%disp(singleab);
%bins = singleab(7);
irf = singleab(2,:);
%bins = singleab(1,:);
...
[f1,gof,output] = fit(x',irf',g,'Lower',[1 0 1],'upper',[60 6 10]);
disp(gof);
figure(111)
plot(f1,x,irf);
%plot(x,irf);
