...
threshold1 = baseline1+15*mad1;
%threshold1 = baseline1+10*mad1; %Run flowhist first to determine threshold for peak finding. Input threshold for trajectory 1(membrane dye).
B(:,1)=data1;
...
mad2 = mad(data2,1);
threshold2 = baseline2+foldFit*mad2; %Run flowhist first to determine threshold for peak finding.
B(:,1)=data2;
...
whole_area=a;
disp([a, b, c]);
cutoff = foldFit * mad2/interval;% change it to the cutoff point
max1 = max(sumi3)/interval;
detection_eff=sum(f1(cutoff:max1))/whole_area*100;
f = [foldFit, gof.rsquare, detection_eff];
end
